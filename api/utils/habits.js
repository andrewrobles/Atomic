...
// TODO: combine redundant logic in getCalendar and handleEmptyDates
const getCalendar = (dates, today) => {
    if (dates.length === 0) {
        return handleEmptyDates(today)
    }
...
const handleEmptyDates = (today) => {
    let formattedDates = []
    const max_months = 12
    let monthCount = 1 

    let year = parseInt(today.split('-')[0]);
    let month = parseInt(today.split('-')[1]) - 1; // Convert to 0-based month index
    while (monthCount <= max_months) {
        const daysInMonth = new Date(year, month + 1, 0).getDate();
        const monthData = {};
        const daysArray = [[], [], [], [], [], [], []];
